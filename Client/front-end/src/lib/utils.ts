...
export function getAvatarUrl(avatar?: string): string {
  ...
  if (avatar.startsWith("http://") || avatar.startsWith("https://")) {
    ...
    // Return all other HTTP/HTTPS URLs as-is (Google, ui-avatars, etc.)
    return avatar;
  }

  // For relative /uploads paths, use absolute URL with current host so it goes through reverse proxy
  if (avatar.startsWith("/uploads")) {
    if (typeof window !== "undefined") {
      const { protocol, hostname, port } = window.location;
      return `${protocol}//${hostname}${port ? ':' + port : ''}${avatar}`;
    }
    return avatar; // Server-side fallback
  }
