...
export function ChartAreaDefault({ playerId }: XpGraphProps) {
  ...
  const socket = useSocketStore((state: { socket: any }) => state.socket)
  ...
  const fetchXpHistory = useCallback(async () => {
    try {
      ...
      interface XpEntry { week: string; xp?: number; weekGain?: number; isCurrentMonth?: boolean; }
      // Format data for chart
      const formattedData = xpHistory.map((entry: XpEntry) => ({
        week: entry.week,
        xp: entry.xp || 0,
      ...
      // Find current month
      const currentMonthEntry = formattedData.find((entry: XpEntry) => entry.isCurrentMonth)
