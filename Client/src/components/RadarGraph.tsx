...
export function ChartRadarDefault({ playerId }: RadarGraphProps) {
  ...
  const [globalStats, setGlobalStats] = useState<{ maxRating?: number; maxGames?: number; maxWins?: number; maxConsistency?: number; maxWinRate?: number; playerPercentile?: number } | null>(null)
        ...
        <div className="mt-4 px-4 pb-4 text-center space-y-2 flex-shrink-0">
          ...
          {globalStats && (
            <div className="text-sm text-gray-600 space-y-1">
              <p>
                You&apos;re in the top <span className="font-semibold text-black">{(100 - (globalStats.playerPercentile ?? 50)).toFixed(1)}%</span> of players
              </p>
