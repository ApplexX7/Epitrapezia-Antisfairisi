...
import { useSocketStore, Notification } from "./hooks/SocketIOproviders";
...
export default function HomeNavBar (){
    ...
    const [search, isSearching] = useState(false);
        ...
        const notifications = useSocketStore((state: SocketStore) => state.notifications);
        const markAllAsRead = useSocketStore((state: SocketStore) => state.markAllAsRead);
        const markAsRead = useSocketStore((state: SocketStore) => state.markAsRead);
        ...
        useEffect(() => {
            ...
            const handleFriendRequestHandled = ({ friendId }: { friendId: number; action: string }) => {
                ...
                const updatedNotifications = notifications.map((n: Notification) =>
                    n.from?.id === String(friendId) && n.type === "friend-request" 
        ...
        const handleDismiss = (notifId: string) => {
            const updatedNotifications = notifications.filter((n: Notification) => n.id !== notifId);
            useSocketStore.setState({ notifications: updatedNotifications });
        };

        const handleFriendAction = async (notif: Notification, action: "accept" | "decline") => {
            ...
            try {
                ...
                console.error("Failed to update friend request", axiosError?.response?.data || axiosError?.message || err);
                // If the request no longer exists (404), remove the notification
                if (axiosError?.response?.status === 404) {
                    const notifications = useSocketStore.getState().notifications;
                    const updatedNotifications = notifications.filter((n: Notification) => n.id !== notif.id);
                    useSocketStore.setState({ notifications: updatedNotifications });
                    toast.error("This friend request no longer exists");
                } else {
                    const errorMessage = axiosError?.response?.data?.message || axiosError?.message || "Action failed";
                    toast.error(errorMessage);
                }
            }
        };

        const handleGameInviteResponse = (notif: Notification, status: "accepted" | "declined") => {
                                                ...
                                                {notifications.map((n: Notification) => (
                                                    ...
                                                    >
                                                        <div className="flex items-start justify-between gap-2">
                                                            ...
                                                            <span className="text-[10px] text-gray-600 whitespace-nowrap">{n.time && formatTime(n.time)}</span>
                                                        </div>
