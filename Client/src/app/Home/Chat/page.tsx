...
export default function Home() {
  const [messages, setMessages] = useState<Record<string, FormattedMessage[]>>({});
  ...
  if (!recipient || !socket || !user) return;
  ...
  const newMessage: FormattedMessage = {
    ...
    user: "me",
    seen: false,
    avatar: user.avatar || "/images/defaultimage.png"
  };
...
useEffect(() => {
  ...
  useEffect(() => {
    ...
    const handleChatMessage = (data: { from: string; to: string; content: string; timestamp: number; message_id?: string }) => {
      const sender = friendsRef.current.find(f => f.username === data.from);
      const senderUsername = sender ? sender.username : data.from;
      ...
      setMessages(prev => ({
        ...
        [senderUsername]: [
          ...(prev[senderUsername] || []),
          {
            ...
            user: data.from === user?.username ? "me" : "other",
            seen: false,
            avatar: sender?.avatar || "/images/defaultimage.png",
          },
        ],
      }));
    };
  ...
  const handleSelectChat = async (username: string) => {
...
const handleUnblockUser = async (username: string) => {
  ...
  try {
    ...
    const error = err as { response?: { data?: { message?: string } }; message?: string };
    console.error("❌ Unblock failed:", error.response?.data || error.message);
  ...
  const handleInviteToGame = () => {
    ...
    socket.emit("game:invite", { to: recipient.id, mode: "friendly" }, (resp?: { ok?: boolean; error?: string }) => {
