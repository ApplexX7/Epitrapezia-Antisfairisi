...
export default function Home() {
  ...
  return (
    ...
    items-center bg-[#F5F5F5]/40 rounded-xl m-2 md:m-10">
      <div className={`${showChatList ? 'flex' : 'hidden md:flex'} flex-col w-full md:w-2/3 lg:w-1/3 h-full backdrop-brightness-[120%]
        ...
        <div className="flex-1 space-y-2 md:space-y-4 overflow-y-auto">
          ...
          <div
            ...
            className={`flex items-center gap-2 md:gap-3 m-0 p-3 md:p-4 ${selectedChat === chat.id ? "bg-white/20" : "hover:bg-white/20 cursor-pointer"}`}
          > 
          <div className="relative">
            <Image src="/images/defaultAvatare.jpg" alt="Profile" width={40} height={40}
            className="rounded-full gap-2 ml-3 md:ml-7" />
            <div
              className={`absolute bottom-0 right-0 w-3 h-3 ${chat.online ? "bg-green-500" : "bg-none"} border-2 border-none rounded-full `}>
            </div>
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-white font-medium text-sm md:text-base">{chat.name}</p>
            <p className="text-gray-300 text-xs md:text-sm truncate"> {chat.last} </p>
          </div>
          <span className="text-xs text-gray-200 mr-3 md:mr-7">{time.clock}</span>
          </div>
          ))}
        </div>
      </div>
          ...
          <>
            <div className="md:hidden flex items-center gap-3 p-4 bg-[#D1DAE9]/20 border-b border-white/10">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="font-bold text-lg">{getSelectedChatName()}</span>
              </div>
            </div>
